...
# This script scaffolds the Week 6 MERN backend assignment.
# Idempotent: re-running will not harm existing files (existing files are left untouched).
...
if [ ! -f "$BACKEND_DIR/src/models/bug.js" ]; then
...
if [ ! -f "$BACKEND_DIR/src/utils/validation.js" ]; then
...
if [ ! -f "$BACKEND_DIR/src/controllers/bugsController.js" ]; then
...
if [ ! -f "$BACKEND_DIR/src/routes/bugs.js" ]; then
...
if [ ! -f "$BACKEND_DIR/src/middleware/errorHandler.js" ]; then
...
if [ ! -f "$BACKEND_DIR/tests/validation.test.js" ]; then
...
if [ ! -f "$BACKEND_DIR/tests/bugs.routes.test.js" ]; then
...
echo "Backend scaffold complete under $BACKEND_DIR"
echo "Windows (PowerShell) run examples:"
echo "  bash ./create_week6_backend.sh            # Using Git Bash (no chmod needed)"
echo "  wsl bash ./create_week6_backend.sh        # Using WSL Ubuntu/Debian"
echo "Linux/macOS run:"
echo "  chmod +x create_week6_backend.sh && ./create_week6_backend.sh"
